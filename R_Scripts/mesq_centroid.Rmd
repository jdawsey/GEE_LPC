...
files_list <- list.files(path="F:/!!Research/github_gee_backup/mesq_jsons/")
...
file_address_func <- function(file_given) {
  ...
  for (file in file_given) {
    address <- "F:/!!Research/github_gee_backup/mesq_jsons/"
...
```{r}
assetID <- paste0(ee_get_assethome(), '/mesq_test')
ee_image_to_asset(
  result,
  description = 'mesquite_test',
  assetID = assetID,
...
target_class <- result$select('cluster')$eq(7) ### mesquite class
Map$addLayer(target_class, visParams = mesqPalette)
...
centroids_vector <- centroids_w_count$reduceToVectors(
