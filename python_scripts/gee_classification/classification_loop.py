...
def download_classified_thumbnails(folder_directory, data_folder, shp_dir, 
    ...
    for shp_file in shp_files:
        ...
        if xmeans == True:
            classified_image = ee.Clusterer.wekaXMeans(maxClusters = n_clusters, # change clusters depending on imagery
                                    maxIterations = 5, 
        ...
        for i in range(fishnet.size().getInfo()):
            ...
            cellname = f'{file_name_split[0]}_gridcell_{grid_cell_num}_classified'
...
"""
def download_classified_drive(folder_directory, data_folder, shp_dir, max_pixels, xmeans = False,
                                n_clusters = 10, scale = 1, crs = 'EPSG:6350'):
    ...
    for shp_file in shp_files:
        ...
        # creating a path for the classified image to export to
        cellname = f'{file_name_split[0]}_classified'
