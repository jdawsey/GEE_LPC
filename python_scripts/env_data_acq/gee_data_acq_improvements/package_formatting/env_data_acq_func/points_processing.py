...
def points_to_gdf(points_path, crs):
...
class PointFishnet:
    def __init__(self, points_gdf, bounds, cell_size, max_points, crs="EPSG:4326", max_iterations=10):
    ...
    def count_points_in_cells(self):
        self.grid['points_count'] = self.grid.apply(
            lambda cell: self.points_gdf.within(cell.geometry).sum(),
            axis=1
        )
    ...
    def subdivide_cell(self, cell, points_gdf, max_points):
        if cell.points_count > max_points:
            ...
            sub_fishnet = PointFishnet(points_gdf, (xmin, ymin, xmax, ymax), new_cell_size, max_points, self.crs, self.max_iterations - 1)
            sub_fishnet.count_points_in_cells()
            sub_fishnet.subdivide_high_density_cells()

            return sub_fishnet.grid
        else:
            return gpd.GeoDataFrame([cell], crs=self.crs)

    def subdivide_high_density_cells(self):
        iteration = 0
        need_subdivision = True
        while need_subdivision and iteration < self.max_iterations:
            iteration += 1
            subdivided_cells = gpd.GeoDataFrame(pd.concat([
                self.subdivide_cell(cell, self.points_gdf, self.max_points) for idx, cell in self.grid.iterrows()
            ], ignore_index=True))
            subdivided_cells.set_crs(self.crs, inplace=True)

            max_points_in_cell = subdivided_cells['points_count'].max()
            if max_points_in_cell <= self.max_points:
                need_subdivision = False

            self.grid = subdivided_cells

    def export_points_in_cells(self, output_folder):
        ...
        for idx, cell in self.grid.iterrows():
            cell_num += 1
            intersecting_points = self.points_gdf[self.points_gdf.within(cell.geometry)]
