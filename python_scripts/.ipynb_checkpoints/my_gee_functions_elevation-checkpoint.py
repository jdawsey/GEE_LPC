...
def stdrd_ref_func(ee_item_list):
    ...
    elevation = ee.ImageCollection('USGS/3DEP/1m').filterBounds(shp).mosaic().clip(shp).select(['elevation'])
    #slope = ee.Terrain.slope(elevation).select(['slope']).rename('slope')
    n_clip = naip_savi_endvi(clip)
    n_clip = ee.Image([n_clip, elevation])
...
def segmentation_func(stdrd_image):
    ...
    bands = ['R', 'G', 'B', 'N', 'savi', 'endvi', 'elevation']
    seed_spacing = 5
    ...
    ).select(['R_mean', 'G_mean', 'B_mean', 
              'N_mean', 'savi_mean', 'endvi_mean', 
              'elevation_mean','clusters'], 
             ['R', 'G', 'B', 'N', 'savi', 'endvi', 
              'elevation', 'clusters'])
...
def obj_class(ee_item_given, given_segmented, max_clusters):
    ...
    clusterer = ee.Clusterer.wekaXMeans(maxClusters = max_clusters, # change clusters depending on imagery
                                        ...
                                        """
    #LVQ clusterer
    #clusterer = ee.Clusterer.wekaLVQ(numClusters = max_clusters).train(training)

    # cascade k means
    #clusterer = ee.Clusterer.wekaCascadeKMeans(maxClusters = max_clusters, distanceFunction = 'Manhattan').train(training)
